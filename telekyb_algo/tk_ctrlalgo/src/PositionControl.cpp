...
void PositionControl::run(const TKTrajectory& input, const TKState& currentState, const double mass, PosCtrlOutput& output)
{
    ...
    double comThrust =  ( mass/(cos( (double)curOrientation(0) )*cos( (double)curOrientation(1) )) ) * ( -1.0 * options.tGravity->getValue() + comZ );
    ...
    double comRoll = ( mass / comThrust) * ( -sin((double)curOrientation(2))*comX + cos((double)curOrientation(2))*comY);

    double comPitch = ( mass / (comThrust*cos((double)curOrientation(0)))) * ( cos((double)curOrientation(2))*comX + sin((double)curOrientation(2))*comY);
    ...
    ROS_DEBUG_STREAM_THROTTLE(1,"tk ctrlAlgo PositionControl comRoll " << output.comRoll);
    ROS_DEBUG_STREAM_THROTTLE(1,"tk ctrlAlgo PositionControl comPitch " << output.comPitch);
