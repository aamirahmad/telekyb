...
#include <StateEstimators/KalmanStateEstimator/ViconHandler.hpp>
#include <EigenTools.hpp>


namespace telekyb_state {

ViconHandlerOption::ViconHandlerOption()
	: OptionContainer("ViconHandler")
{
...
void ViconHandler::initialize()
{
	sensorSub = nodeHandle.subscribe(options.vicTopic->getValue(),1, &ViconHandler::measureCallback, this);
	vicPosition = options.vicPosition->getValue();
    vicOrientation = options.vicOrientation->getValue();
}

void ViconHandler::measureCallback(const geometry_msgs::TransformStamped::ConstPtr& msg)
{
	lastMeasure.timeStamp = msg->header.stamp.sec;
	lastMeasure.measure = *msg;
	//boost::mutex::scoped_lock scopedLockMutex(newMeasureMutex);
	//newMeasureReceived = true;
}

// Correction function ====================================================
void ViconHandler::update(StateBufferElement& state, const geometry_msgs::TransformStamped& z)
{
	// Eigen tools;
	EigenTools tools;

	//read the state
	const Eigen::Vector3d  r(state.state.pose.position.x,state.state.pose.position.y,state.state.pose.position.z);
	const Eigen::Vector3d dr(state.state.twist.linear.x,state.state.twist.linear.y,state.state.twist.linear.z);
	const Eigen::Quaterniond q(state.state.pose.orientation.w,state.state.pose.orientation.x,state.state.pose.orientation.y,state.state.pose.orientation.z);

	//read the measure
	const Eigen::Vector3d  zr(state.state.pose.position.x,state.state.pose.position.y,state.state.pose.position.z);
	const Eigen::Quaterniond zq(state.state.pose.orientation.w,state.state.pose.orientation.x,state.state.pose.orientation.y,state.state.pose.orientation.z);
	//TODO covariance should be in the message
	const Eigen::Matrix<double, 6, 6> Qt = options.vicCov->getValue();

	const Eigen::Vector3d BrBM = vicPosition;
	const Eigen::Quaterniond BqM = vicOrientation;
