/*
 * SMURFTrajectoryTrackerExtForceCompensated.cpp
 *
 *  Created on: Jun 8, 2012
 *      Author: rspica
 */

#include "SMURFTrajectoryTrackerExtForceCompensated.hpp"
...
PLUGINLIB_EXPORT_CLASS( trajectory_trackers_plugin::SMURFTrajectoryTrackerExtForceCompensated, TELEKYB_NAMESPACE::TrajectoryTracker);

namespace trajectory_trackers_plugin {

// Options
SMURFTrajectoryTrackerExtForceCompensatedOptions::SMURFTrajectoryTrackerExtForceCompensatedOptions()
	: OptionContainer("SMURFTrajectoryTrackerExtForceCompensated")
{
	...
	tExternalWrenchFromTopic = addOption<bool>("tExternalWrenchFromTopic","If true, the compensated external wrench is read from tExternalWrenchTopic ros topic. If false, it is read from the tkTrajectory.",true,false,false);
	tExternalWrenchTopic = addOption<std::string>("tExternalWrenchTopic","Topic for Subscribing the input wrench","/Telekyb/DisturbanceWrench",false,false);
...
SMURFTrajectoryTrackerExtForceCompensated::SMURFTrajectoryTrackerExtForceCompensated()
...
void SMURFTrajectoryTrackerExtForceCompensated::initialize()
{
	...
	extTorque << 0.0,0.0,0.0;
	if (options.tExternalWrenchFromTopic->getValue()){
		externalWrenchSub = nodeHandle.subscribe(options.tExternalWrenchTopic->getValue(),1,&SMURFTrajectoryTrackerExtForceCompensated::externalWrenchCB,this);
	}
}

void SMURFTrajectoryTrackerExtForceCompensated::destroy(){
}

std::string SMURFTrajectoryTrackerExtForceCompensated::getName() const{
	return "SMURFTrajectoryTrackerExtForceCompensated";
}

void SMURFTrajectoryTrackerExtForceCompensated::trajectoryCB(const TKTrajectory& trajectory){
	boost::mutex::scoped_lock refTrajectoryLock(refTrajectoryMutex);
	refTrajectory = trajectory;
	// if the external wrench is not communicated through independent topic, then the jerk of the reference trajectory is interpreted as force to be compensated;
	if(!options.tExternalWrenchFromTopic->getValue()){
		extForce(0) = refTrajectory.jerk[0];
		extForce(1) = -refTrajectory.jerk[1];
		extForce(2) = -refTrajectory.jerk[2];
	}
}

void SMURFTrajectoryTrackerExtForceCompensated::externalWrenchCB(const geometry_msgs::WrenchStamped& externalwrench){
	extForce(0) = externalwrench.wrench.force.x;
	extForce(1) = -externalwrench.wrench.force.y; // -ve sign added to change from NWU to NED
	extForce(2) = -externalwrench.wrench.force.z; // -ve sign added to change from NWU to NED
	
	extTorque(0) = externalwrench.wrench.torque.x;
	extTorque(1) = -externalwrench.wrench.torque.y; // -ve sign added to change from NWU to NED
	extTorque(2) = -externalwrench.wrench.torque.z;	// -ve sign added to change from NWU to NED
}  

void SMURFTrajectoryTrackerExtForceCompensated::stateCB(const TKState& state){
...
void SMURFTrajectoryTrackerExtForceCompensated::run(const TKTrajectory& input, const TKState& currentState, const double mass, const Eigen::Matrix3d& inertia, Eigen::Vector4d& ctrlInputs){
	...
	if (curPosition(2) >= -0.6){
	  extForce << 0.0,0.0,0.0;
        }
	const Eigen::Vector3d Fd =  gainKip.asDiagonal()*posIntState + gainKp.asDiagonal()*(rt-r) + gainKv.asDiagonal()*(drt-dr) + mass*(ddrt - gravity) - extForce;
...
void SMURFTrajectoryTrackerExtForceCompensated::saturateUniform(Eigen::Vector4d& ctrlInputs, double mass){
...
void SMURFTrajectoryTrackerExtForceCompensated::runGripperCtrl(const TKTrajectory& input, const TKState& currentState, const double mass, const Eigen::Matrix3d& inertia, Eigen::Vector4d& inputs){
