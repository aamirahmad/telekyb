...
void TakeOff::initialize()
{
	tTakeOffDestination = addOption<Position3D>("tTakeOffDestination",
			...
			Position3D(0.0,0.0,-1.0), false, true);
...
{
	startingPosition = currentState.position;
	startingTime = Time().toDSec();
...
void TakeOff::trajectoryStepActive(const TKState& currentState, TKTrajectory& generatedTrajInput)
{
	...
	if (tTakeOffInPosition->getValue()){
		double elapsed = Time().toDSec() - startingTime;
		Position3D newSetPoint = startingPosition + (actualTakeOffDestination - startingPosition).normalized() * tTakeOffVelocity->getValue()*elapsed;
		if ((newSetPoint-startingPosition).norm()>(actualTakeOffDestination-startingPosition).norm()){
			generatedTrajInput.setVelocity(Velocity3D(0.0,0.0,0.0));
			generatedTrajInput.setPosition(actualTakeOffDestination);
		}
		else{
			generatedTrajInput.setVelocity((actualTakeOffDestination - startingPosition).normalized()* tTakeOffVelocity->getValue());
			generatedTrajInput.setPosition(startingPosition + (actualTakeOffDestination - startingPosition).normalized() * tTakeOffVelocity->getValue()*elapsed);
		}
		
// 		Velocity3D direction = actualTakeOffDestination - currentState.position;
// 		generatedTrajInput.setVelocity(direction.normalized()* tTakeOffVelocity->getValue());
// 		generatedTrajInput.setPosition(currentState.position + 0.05*direction.normalized()* tTakeOffVelocity->getValue());
	}
...
bool TakeOff::isValid(const TKState& currentState) const
{
	...
	//std::cout << " actualTakeOffDestination " << actualTakeOffDestination(0) << " " << actualTakeOffDestination(1) << " " << actualTakeOffDestination(2) << " currentState.position  "  << currentState.position(0) << " " << currentState.position(1) << " " << currentState.position(2) << std::endl;
