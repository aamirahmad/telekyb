...
void Land::trajectoryStepActive(const TKState& currentState, TKTrajectory& generatedTrajInput)
{
	...
	if (tLandInPosition->getValue()){
		...
		generatedTrajInput.setVelocity((actualLandDestination - startingPosition).normalized()* tLandVelocity->getValue());
		//generatedTrajInput.setPosition(actualLandDestination);
		generatedTrajInput.setPosition(startingPosition + (actualLandDestination - startingPosition).normalized() * tLandVelocity->getValue()*elapsed);
	}
}
