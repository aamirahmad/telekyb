...
/*	for (int i = 0; i < tokenList.size(); ++i) {
	...
	if (tokenList.size() < 25) {
		ROS_ERROR("Line has less than 25 Tokens!");
		return false;
	}
	try {
		//ROS_INFO("Found 25 or more tokens per line, will use new trajectory file definition including jerk and snap.");
		nextTimeStep = boost::lexical_cast<double>(tokenList[0]);
		...
		AngVelocity3D angvelocity(
				boost::lexical_cast<double>(tokenList[19]),
				boost::lexical_cast<double>(tokenList[20]),
				boost::lexical_cast<double>(tokenList[21])
		);
		
		AngVelocity3Ddot angvelocitydot(
				boost::lexical_cast<double>(tokenList[22]),
				boost::lexical_cast<double>(tokenList[23]),
				boost::lexical_cast<double>(tokenList[24])
		);

		nextTrajInput.set3DPose(position, velocity, acceleration, jerk, snap, rotangle, angvelocity, angvelocitydot);
...
// called everytime a new TKState is available. Should return false if invalid (switch to next behavior, or Hover if undef).
