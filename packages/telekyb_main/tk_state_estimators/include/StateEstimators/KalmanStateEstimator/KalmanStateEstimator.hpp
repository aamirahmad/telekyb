...
#include <StateEstimators/ViconHandler.hpp>
...
protected:
	...
	typename std::deque<_BufferElement>::iterator searchInBuffer(std::deque<_BufferElement>& buffer, double time);

};


template <typename _BufferElement>
typename std::deque<_BufferElement>::iterator KalmanStateEstimator::searchInBuffer(std::deque<_BufferElement>& buffer, double time) {
	typename std::deque<_BufferElement>::iterator current;
