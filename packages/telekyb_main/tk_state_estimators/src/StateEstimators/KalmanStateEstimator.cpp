	...
	: OptionContainer("Kalman")
{
	...
	Eigen::Matrix<double,9,9> dt;
	dt.Zero();
	discCov = addOption<Eigen::Matrix<double,9,9> >("discCov", "Discretization noise covariance matrix", dt, false, true);

	Eigen::Matrix<double,9,9> initC;
	initC.Zero();
	initC.block<3,3>(0,0) = qt.block<3,3>(0,0);
	initC.block<3,3>(3,3) = 10*qt.block<3,3>(0,0);
	initC.block<3,3>(6,6) = qt.block<3,3>(3,3);
...
void KalmanStateEstimator::core()
{
	...
	if (newInputReceived)
	{
		...
		if ((int)inputBuffer.size()==options.maxInputBufferSize->getValue()) inputBuffer.pop_front();
		inputBuffer.push_back(newInput);

		//syncronize internal time with the IMU
	...
	{
		//Check if the current measure is newer than the oldest saved state.
		std::deque<StateBufferElement, Eigen::aligned_allocator<StateBufferElement> >::iterator stateBufferIndex;
		std::deque<InputBufferElement, Eigen::aligned_allocator<InputBufferElement> >::iterator inputBufferIndex;
		std::deque<MeasureBufferElement, Eigen::aligned_allocator<MeasureBufferElement> >::iterator measureBufferIndex;
		...
		{
			ROS_WARN("A measure has been neglected because it was too old. Consider increasing the size of the state and input buffers");
		} else {
			// add measure to the buffer (delete oldest entry if the bus is too big)
	...
	while (true)
	{
		...
		switch (condition)
		{
			...
			case (3): //update is required
				viconHanler.update(internalState, *nextMeasureIndex);
				nextMeasureIndex++;
				if (nextMeasureIndex == measureBuffer.end())
					{
						nextMeasureTime = INFINITY;
...
void KalmanStateEstimator::willBecomeActive()
{
	...
	vicSub = nodeHandle.subscribe(options.viconTopic->getValue(),1, &KalmanStateEstimator::viconCallback, this);
