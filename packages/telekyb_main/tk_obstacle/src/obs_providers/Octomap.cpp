	...
	: OptionContainer("Octomap")
{
	...
	tFileName = addOption<std::string>("tFileName",
			...
			"/home/paolo/telekyb_ws/src/packages/telekyb_gazebo/resource/power_plant.bt",false,false);
}
...
void Octomap::getObstaclePoints(const TKState& lastState, std::vector<Position3D>& obstaclePoints) const
{
  ...
  double x, y, z;
  for (x = xq-ray; x<=xq+ray; x+=res){
    for (y = yq-ray; y<=yq+ray; y+=res){
      for (z = zq-ray; z<=zq+ray; z+=res){
