/**
 * TeleoperationTrajectoryTracker.cpp
 *
 ...
 *  
 */
#include "TeleoperationTrajectoryTracker.hpp"
...
PLUGINLIB_EXPORT_CLASS( trajectory_trackers_plugin::TeleoperationTrajectoryTracker, TELEKYB_NAMESPACE::TrajectoryTracker);

namespace trajectory_trackers_plugin {

// Options
TeleoperationTrajectoryTrackerOptions::TeleoperationTrajectoryTrackerOptions()
	: OptionContainer("TeleoperationTrajectoryTracker")
{
...
TeleoperationTrajectoryTracker::TeleoperationTrajectoryTracker() : tDoMassEstimation( NULL ),
...
TeleoperationTrajectoryTracker::~TeleoperationTrajectoryTracker() {
	if (massEstimator) {
...
void TeleoperationTrajectoryTracker::initialize() {
...
void TeleoperationTrajectoryTracker::destroy() {
	delete positionControl;
	delete yawControl;
}

std::string TeleoperationTrajectoryTracker::getName() const {
	return "TeleoperationTrajectoryTracker";
}

void TeleoperationTrajectoryTracker::trajectoryCB(const TELEKYB_NAMESPACE::TKTrajectory& trajectory) {
	boost::mutex::scoped_lock currentInputLock(currentInputMutex);
	currentInput = trajectory;
}

void TeleoperationTrajectoryTracker::stateCB(const TELEKYB_NAMESPACE::TKState& state) {
...
//	ROS_INFO("TeleoperationTrajectoryTracker::stateCB acc %f %f %f", currentInput.acceleration(0), currentInput.acceleration(1), currentInput.acceleration(2));

//	ROS_INFO("TeleoperationTrajectoryTracker::stateCB Roll: %f, Pitch: %f, Thrust %f", pcOutput.comRoll, pcOutput.comPitch, pcOutput.comThrust);
	...
	if (currentInput.jerk(0) > 0) {
		cmdMsg.roll = currentInput.acceleration(0);
	} else {
		cmdMsg.roll = pcOutput.comRoll;
	}

//	if (currentInput.jerk(1) > 0) {
//		cmdMsg.pitch = currentInput.acceleration(1);
//	} else {
