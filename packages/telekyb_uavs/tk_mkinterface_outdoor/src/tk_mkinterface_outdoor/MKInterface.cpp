...
#include <tk_mkinterface_outdoor/MKCalibrator.hpp>
	...
	: safeModule(NULL), connection(NULL), rosInterface(NULL), batteryFilter(NULL), battery(NULL), receivedFirstBatteryLevel(false), batteryFiltered(-1.0)
{
	...
	if (connection) {
		...
		if (options.tUavFirmware->getValue() > 1641){
			...
			} else if (options.tCommandType->getValue() == CommandType::tu2u3u4){
				if (!connection->setValue(MKSingleValuePacket(MKDataDefines::FLIGHT_CTRL_MODE, MKDataDefines::SMURF_CTRL_MODE))) {
					ROS_ERROR("Could not set FLIGHT_CTRL_MODE!");
				} else {
					ROS_INFO("Using SMURF controller with thrust/torques reference");
				}
			} else {
...
double MKInterface::getAtmoPress(MKInt& atmoPressValue) 
{
// 	atmoPress = connection->getMKDataRef().getValueByID(MKDataDefines::RAW_ATMO_PRESS);
// 	if (connection->updateValue(MKDataDefines::RAW_ATMO_PRESS)) {
// 		atmoPressValue = atmoPress->getValue(); 
// 	}
}
