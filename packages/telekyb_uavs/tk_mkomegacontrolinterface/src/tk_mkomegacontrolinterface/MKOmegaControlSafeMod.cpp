...
MKOmegaControlSafeModOptions::MKOmegaControlSafeModOptions()
	: OptionContainer("MKOmegaControlSafeMod")
{
...
//			"Thread sleep time in us of MKOmegaControlSafeMod",
//			1000, 100, 50000, false, true); // must be less then a second
}

MKOmegaControlSafeMod::MKOmegaControlSafeMod(MKSafeModDeleagte* delegate_, MKOmegaControlInterfaceConnection* connection_)
	: connection(connection_), active(false), delegate(delegate_), mkSafeModThread(NULL), emergencyLandingRequest(false)
{

}

MKOmegaControlSafeMod::~MKOmegaControlSafeMod()
{
	if (mkSafeModThread) {
		stopThread();
	}
...
void MKOmegaControlSafeMod::safeModFcn()
{
	active = true;
...
void MKOmegaControlSafeMod::threadFcn()
{
...
void MKOmegaControlSafeMod::startThread()
{
	if (!mkSafeModThread) {
		threadStopRequest = false;
		mkSafeModThread = new boost::thread(&MKOmegaControlSafeMod::threadFcn, this);
	} else {
		ROS_WARN("Trying to start MKOmegaControlSafeModThread, but it's already active!");
	}

}

void MKOmegaControlSafeMod::stopThread()
{
	if (mkSafeModThread) {
		threadStopRequest = true;
		...
		ROS_WARN("Trying to stop MKOmegaControlSafeModThread, but it does not exist!");
	}
}

void MKOmegaControlSafeMod::start()
{
	startThread();
}

void MKOmegaControlSafeMod::stop()
{
	stopThread();
}

// returns true if SafeMod has Taken Over
bool MKOmegaControlSafeMod::isActive() const
{
	return active;
}

bool MKOmegaControlSafeMod::isRunning() const
{
	return (mkSafeModThread != NULL);
}

void MKOmegaControlSafeMod::resetCmdTimer()
{
  cmdTimer.reset();
}

void MKOmegaControlSafeMod::setEmergency()
{
