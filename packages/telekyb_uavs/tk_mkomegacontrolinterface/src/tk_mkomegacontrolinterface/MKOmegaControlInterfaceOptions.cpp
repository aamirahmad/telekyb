/*
 * MKOmegaControlInterfaceOptions.cpp
 *
...
MKOmegaControlInterfaceOptions::MKOmegaControlInterfaceOptions()
	: OptionContainer("MKOmegaControlInterface")
{
#ifdef __APPLE__
	...
	tInitialDriftEstimPeriod = addOption<double>("tInitialDriftEstimPeriod",
		"Number of seconds allowed for initial onboad gyro drift estimation", 2, false, true);
	tIMUOffsetEstimationSamples = addOption<double>("tIMUOffsetEstimationSamples",
		"Number of samples used in the estimation of the IMU offset", 200, false, true);

	tForceToVelConstant = addOption<double>("tForceToVelConstant",
		"Estimated constant (k) that relates the square of the motor velocity to the propeller force: f_i = k*omega_i^2", 0.00065, false, true);
