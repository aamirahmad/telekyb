...
public:
    ...
    void setGeometric(
        ...
        // full 3D pose setter, added by Paolo Stegagno
    void set3DPose(
            const Position3D& position_,
            const Velocity3D& velocity_ = Velocity3D::Zero(),
            const Acceleration3D& acceleration_ = Acceleration3D::Zero(),
            ...
            const RotAngle3D& rotangle_ = Eigen::Vector3d::Zero(),
            const AngVelocity3D& angvelocity_ = Velocity3D::Zero(),
            const AngVelocity3Ddot& angvelocitydot_ = Velocity3D::Zero()
            );
...
void TKTrajectory::set3DPose(
	...
	const RotAngle3D& rotangle_,
	const AngVelocity3D& angvelocity_,
	const AngVelocity3Ddot& angvelocitydot_
	)
{
    position = position_;
    velocity = velocity_;
    ...
    rotangle = rotangle_;
    angvelocity = angvelocity_;
    angvelocitydot = angvelocitydot_;

    anglesCtrl = PosControlType::Full3DPose;
}
...
GlobalPosControlType TKTrajectory::getGlobalPositionControlType() const
{
    ...
    } else if (anglesCtrl == PosControlType::Geometric) {
        retValue = GlobalPosControlType::Geometric;         //Geometric tracking setter, added by Yuyi Liu
    } else if (anglesCtrl == PosControlType::Full3DPose) {
        retValue = GlobalPosControlType::Full3DPose;         //Geometric tracking setter, added by Yuyi Liu
    } else 	{
        // Mixed
    }
