      ...
      frame_id("")
{
    ...
    anglesCtrl = PosControlType::Geometric;
...
void TKTrajectory::toTKTrajMsg(telekyb_msgs::TKTrajectory& tkTrajMsg) const
{
    ...
    // angular velocity dot parameter, added by Paolo Stegagno
    tkTrajMsg.angvelocitydot.x = angvelocitydot(0);
    tkTrajMsg.angvelocitydot.y = angvelocitydot(1);
    tkTrajMsg.angvelocitydot.z = angvelocitydot(2);
    // control mode option for geometeric tracking, added by Yuyi Liu
    tkTrajMsg.anglesCtrlType = anglesCtrl.index();
...
void TKTrajectory::fromTKTrajMsg(const telekyb_msgs::TKTrajectory& tkTrajMsg)
{
    ...
    // input desired angular velocity, added by Paolo Stegagno
    angvelocitydot(0) = tkTrajMsg.angvelocitydot.x;
    angvelocitydot(1) = tkTrajMsg.angvelocitydot.y;
    angvelocitydot(2) = tkTrajMsg.angvelocitydot.z;

    // control mode option for geometric tracking mode, added by Yuyi Liu
    anglesCtrl = *PosControlType::get_by_index(tkTrajMsg.anglesCtrlType % PosControlType::size);
